<?php
  // include("config.php");
   ...
   $error=' ';
   $errors = [' ',' ',' ',' ',' ',' ',' ',' '];
   if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["ArchiveCh"]) ) {
	  
	  $selected_channel=$_POST['selected_channel'];
	//   $selected_user=$_POST['selected_user'];
		
		$sql1 = "SELECT channel_id FROM channels WHERE channel_name = '$selected_channel'";
		  $result1 = mysqli_query($db,$sql1);
		  $row1 = mysqli_fetch_array($result1,MYSQLI_ASSOC);
		  $count1 = mysqli_num_rows($result1);
		  $ch_id = $row1['channel_id'];	
		
		$sql = "SELECT * FROM archive WHERE channel_id = '$ch_id'";
		...
		/*  $sql1 = "SELECT * FROM invitation WHERE receiver_name = '$selected_user' AND channel_name = '$selected_channel'";
		  ...
		  else if ($selected_channel == "Select List"){
...
<body class="login-wrap">
  <div>
	<div class="login-html">
		<input id="tab-2" type="radio" name="tab" class="sign-up" checked><label for="tab-2" class="tab">Archive channel</label>
		<div class="login-form">
			<div class="sign-up-htm">
				<form action = "" method = "post" autocomplete="off" enctype="multipart/form-data">
					...
					<br><br>
					<div class="group">
						<input name="ArchiveCh" type="submit" class="button" value="Archive Channel" >
						<br><br>
